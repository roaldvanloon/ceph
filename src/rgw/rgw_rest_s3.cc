 ...
 */
int RGW_Auth_S3_Keystone_ValidateToken::validate_s3token(string auth_id, string auth_token, string auth_sign) {
  ...
  if (ret < 0) {
    dout(10) << "s3 keystone: token validation ERROR: " << rx_buffer.c_str() << dendl;
    return -EPERM;
  }

  /* now parse response */
  if (token.parse(cct, rx_buffer) < 0) {
    dout(10) << "s3 keystone: token parsing failed" << dendl;
    return -EPERM;
  }
  ...
  for (iter = roles_list.begin(); iter != roles_list.end(); ++iter) {
    ...
    if (token.roles.find(role) != token.roles.end()) {
      found = true;
      break;
    }
  }
  ...
  if (!found) {
    ldout(cct, 0) << "s3 keystone: user does not hold a matching role; required roles: " << cct->_conf->rgw_keystone_accepted_roles << dendl;
    return -EPERM;
  }

  /* everything seems fine, continue with this user */
  ldout(cct, 0) << "s3 keystone: validated token: " << token.tenant_name << ":" << token.user_name << " expires: " << token.expiration << dendl;
  return 0;
}
...
int RGW_Auth_S3::authorize(RGWRados *store, struct req_state *s)
{
  ...
  if (!store->ctx()->_conf->rgw_s3_use_rados
      && !store->ctx()->_conf->rgw_s3_use_keystone) {
  ...
  if (store->ctx()->_conf->rgw_s3_use_keystone
    ...
    if (!rgw_create_s3_canonical_header(s->info, &s->header_time, token, qsr)) {
      ...
      if (keystone_result == 0) {
        s->user.user_id = keystone_validator.token.tenant_id;
        s->user.display_name = keystone_validator.token.tenant_name;
        s->perm_mask = RGW_PERM_FULL_CONTROL;
      }
    }
  }
